...
class MainActivity : Activity() {
    ...
    private val titles = linkedMapOf(
        "portfolio" to "PORTFOLIO", "alerts" to "ALERTS", "news" to "NEWS",
        "growth" to "GROWTH", "knowledge" to "KNOWLEDGE", "analysis" to "ANALYSIS",
        "watchlist" to "WATCHLIST", "journal" to "JURNAL ACTIVITATE"
    )
    ...
    private fun showHub() {
        ...
        val scroll=ScrollView(this).apply { isFillViewport=false; setBackgroundColor(Color.rgb(1,3,8)); overScrollMode=View.OVER_SCROLL_NEVER }
        val page=LinearLayout(this).apply { orientation=LinearLayout.VERTICAL; setPadding(dp(10),0,dp(10),0) }
        // Start: map + READY only. No journal card and no fake header controls.
        val hero=OracleHeroView(this){ openModule(it) }
        val heroHeightPx=(resources.displayMetrics.heightPixels*.72f).toInt().coerceAtLeast(dp(560))
        ...
        page.addView(status,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(8),0,0)})
        scroll.addView(page); root.addView(scroll,FrameLayout.LayoutParams(-1,-1))
    }
    private fun dp(v:Int)= (v*resources.displayMetrics.density).toInt()
    private fun openModule(key:String){
        currentModule=key;runCatching{renderModule(key)}.onFailure{showModuleError(key,it)}
        Thread{val result=runCatching{OracleLocalProcessor.refresh(repository)};mainHandler.post{if(currentModule!=key||isFinishing)return@post;result.onSuccess{runCatching{renderModule(key)}.onFailure{showModuleError(key,it)}}.onFailure{e->Toast.makeText(this,"Refresh local eșuat: ${e.message?:e.javaClass.simpleName}",Toast.LENGTH_LONG).show()}}}.start()
    }
    ...
    private fun renderModule(key:String){
        root.removeAllViews()
        val host=OracleNativeModule(context=this,title=titles[key]?:key.uppercase(),onBack={showHub()},onRefresh={refreshModule(key)})
...
private class OracleHeroView(context:android.content.Context,private val onModule:(String)->Unit):View(context){
    ...
    private fun drawNode(c:Canvas,x:Float,y:Float,rad:Float,n:Node,d:Float){
        p.style=Paint.Style.FILL;p.color=Color.argb(244,3,6,13);c.drawCircle(x,y,rad,p);p.style=Paint.Style.STROKE;p.strokeWidth=2.0f*d;p.color=n.color;c.drawCircle(x,y,rad,p);p.style=Paint.Style.FILL;p.color=n.color;c.drawCircle(x,y-rad*.72f,rad*.038f,p);drawIcon(c,x,y-rad*.25f,rad*.29f,n.key,n.color,d);p.textAlign=Paint.Align.CENTER;p.typeface=Typeface.DEFAULT_BOLD;p.textSize=rad*.27f;p.color=n.color;c.drawText(n.label,x,y+rad*.17f,p);p.textSize=rad*.105f;p.color=Color.WHITE;val desc=when(n.key){"portfolio"->"Performanță și poziții";"alerts"->"Semnale și evenimente";"news"->"Știri financiare";"growth"->"Acțiuni cu potențial";"knowledge"->"Idei și documentație";"analysis"->"Analiză detaliată";else->"Acțiuni favorite"};c.drawText(desc,x,y+rad*.42f,p);p.textSize=rad*.28f;c.drawText("›",x,y+rad*.73f,p)
    }

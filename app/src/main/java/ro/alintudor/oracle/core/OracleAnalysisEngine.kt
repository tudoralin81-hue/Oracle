...
object OracleAnalysisEngine {
    ...
    fun analyze(raw:String):Result? {
        ...
        val atr=atr(high,low,close,14)?:p*.01;val atrPct=100*atr/p;val adx=adx(high,low,close,14);val macdPair=macd(close);val week52=close.take(252)
        ...
        val rawValues=listOf("${news.headlineCount} titluri • +${news.positiveHits} pozitiv • -${news.negativeHits} negativ • scor ${news.score}/100",if(p>prior20)"BREAKOUT: DA • R20 ${money(prior20)} • VR %.2fx".format(Locale.US,vr)else"BREAKOUT: NU • R20 ${money(prior20)} • VR %.2fx".format(Locale.US,vr),"SMA20 ${money(s20)} • SMA50 ${money(s50)} • SMA200 ${money(s200)} • Preț ${money(p)}","5D %.2f%% • 20D %.2f%%".format(Locale.US,m5,m20),"Volum %.2fx față de media 20D".format(Locale.US,vr),"Support ${money(lo)} • Resistance ${money(hi)}",fundamentals?.rawText?:"Date fundamentale indisponibile", "Lower ${money(mid?.minus(2*(sd?:0.0)))} • Mid ${money(mid)} • Upper ${money(mid?.plus(2*(sd?:0.0)))} • Poziție %.1f%%".format(Locale.US,bbPos*100),"Tenkan/Kijun ${if(ichi)"bullish"else"neconfirmat/bearish"}",marketContext.rawText,"ATR %.2f%%".format(Locale.US,atrPct),"ADX(14) ${money(adx)}")
        return Result(ticker,p,ss,ms,ls,signal,risk,allocation,resolvedSector,null,rsi,m5,m20,vr,s50,s200,adx,atrPct,atr,macdPair.first,macdPair.second,week52.maxOrNull(),week52.minOrNull(),fundamentals,factors,rawValues)
    }

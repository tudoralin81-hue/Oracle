...
object OracleAnalysisEngine {
    ...
    data class Result(
        ...
        val momentum20D:Double,val volumeRatio:Double,val sma50:Double?,val sma200:Double?,val adx:Double?,val atrPct:Double,val factors:List<Double>,val rawValues:List<String>
    )
    ...
    fun analyze(raw:String):Result? {
        ...
        val ss=score(shortWeights);val ms=score(mediumWeights);val ls=score(longWeights);val overextension=((rsi-65.0)/15.0).coerceIn(0.0,1.0);val volatility=(atrPct/8.0).coerceIn(0.0,1.0);val volumeShock=((vr-1.0)/2.0).coerceIn(0.0,1.0);val acceleration=(abs(m5)/20.0).coerceIn(0.0,1.0);val riskScore=(100.0*(overextension*.30+volatility*.35+volumeShock*.15+acceleration*.20)).coerceIn(0.0,100.0);val risk=when{riskScore>=65->"RIDICAT";riskScore>=35->"MEDIU";else->"SCĂZUT"};val conviction=((ss+ms+ls)/3.0).coerceIn(0.0,100.0);val rawAllocation=1+(conviction/100)*7;val riskFactor=when(risk){"RIDICAT"->.55;"MEDIU"->.78;else->1.0};val allocation=(rawAllocation*riskFactor).coerceIn(1.0,8.0).roundToInt().toDouble();val signal=when{ss>=85->"STRONG BUY";ss>=75->"BUY";ss>=65->"HOLD";ss>=55->"WATCH";else->"AVOID"}
        val rawValues=listOf("${news.headlineCount} titluri • +${news.positiveHits} pozitiv • -${news.negativeHits} negativ • scor ${news.score}/100",if(p>prior20)"BREAKOUT: DA • R20 ${money(prior20)} • VR %.2fx".format(Locale.US,vr)else"BREAKOUT: NU • R20 ${money(prior20)} • VR %.2fx".format(Locale.US,vr),"SMA20 ${money(s20)} • SMA50 ${money(s50)} • SMA200 ${money(s200)} • Preț ${money(p)}","5D %.2f%% • 20D %.2f%%".format(Locale.US,m5,m20),"Volum %.2fx față de media 20D".format(Locale.US,vr),"Support ${money(lo)} • Resistance ${money(hi)}",fundamentals?.rawText?:"Date fundamentale indisponibile — fără valoare sintetică","Lower ${money(mid?.minus(2*(sd?:0.0)))} • Mid ${money(mid)} • Upper ${money(mid?.plus(2*(sd?:0.0)))} • Poziție %.1f%%".format(Locale.US,bbPos*100),"Tenkan/Kijun ${if(ichi)"bullish"else"neconfirmat/bearish"}",marketContext.rawText,"ATR %.2f%% • scor R/R %.1f/100".format(Locale.US,atrPct,rr),"ADX(14) ${money(adx)} • scor Oracle %.1f/100".format(Locale.US,adxScore))
        return Result(ticker,p,ss,ms,ls,signal,risk,allocation,resolvedSector,null,rsi,m5,m20,vr,s50,s200,adx,atrPct,factors,rawValues)
    }

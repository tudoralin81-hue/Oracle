...
object OracleGrowthEngine {
    ...
    fun run(seed:List<OracleGrowthRecommendation> = emptyList()):List<OracleGrowthRecommendation>{
        ...
        for(h in listOf("SHORT","MEDIUM","LONG")){
            ...
            val canonical=OracleAnalysisEngine.analyze(pick.ticker)
            out+=OracleGrowthRecommendation(horizon=h,ticker=pick.ticker,company=meta?.company?:pick.ticker,sector=meta?.sector?:"US",score=score,signal=rating(score),risk=canonical?.risk?:pick.risk,allocationMax=canonical?.allocation?:pick.allocation,forecastPct=pick.forecast[h.lowercase(Locale.US)]?:0.0,momentum5D=pick.mom5,momentum20D=pick.mom20,weights=weights[h]!!.toList(),newsTitle=meta?.newsTitle?:"",newsSource=meta?.newsSource?:"",referenceTimestamp=meta?.referenceTimestamp?:0L,currentPrice=pick.price,adx=pick.adx,factorValues=keys.map{pick.components[it]?:50.0},factorScore=score.toDouble(),generatedAt=System.currentTimeMillis(),source="ORACLE_ENGINE_V5.9.7")
        }
        return out
    }

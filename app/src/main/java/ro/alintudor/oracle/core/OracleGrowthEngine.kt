...
object OracleGrowthEngine {
    ...
    private fun evaluate(t:String,d:List<OracleOhlcvPoint>):C? {
        ...
        val trend=(50+(if(s20!=null&&p>s20)16 else -16)+(if(s50!=null&&p>s50)17 else -17)+(if(s200!=null&&p>s200)17 else -17)).coerceIn(0.0,100.0)
    ...
    private fun tie(c:C,h:String):Double = when(h){"SHORT"->min(2.0,max(-2.0,c.mom5*.15))+min(1.0,max(-1.0,(c.vr-1)*.8))+(if((c.macdHist?:0)>0).5 else 0.0)+(if(c.rsi in 52.0..72.0).5 else 0.0)+(if((c.rsi?:50)>78)-1.0 else 0.0);"MEDIUM"->min(2.0,max(-2.0,c.mom20*.08))+(if(c.ichi).7 else 0.0)+(if((c.adx?:0)>=20).5 else 0.0)+(if((c.macdHist?:0)>0).3 else 0.0)+(if(c.rsi in 50.0..70.0).5 else 0.0)+(if((c.rsi?:50)>78)-.8 else 0.0);else->min(2.0,max(-2.0,c.mom20*.06))+(if(c.ichi).8 else 0.0)+(if((c.adx?:0)>=25).5 else 0.0)+(if((c.macdHist?:0)>0).3 else 0.0)+(if(c.sma200!=null&&c.price>c.sma200).8 else 0.0)+(if(c.sma50!=null&&c.price>c.sma50).4 else 0.0)+(if(c.rsi in 48.0..68.0).4 else 0.0)+(if((c.rsi?:50)>78||c.mom5>15)-.8 else 0.0)}

...
object OracleMarketCalendar {
    ...
    fun isTradingDay(date: LocalDate): Boolean = date.dayOfWeek != DayOfWeek.SATURDAY &&
        date.dayOfWeek != DayOfWeek.SUNDAY && fullClosureName(date) == null

    fun fullClosureName(date: LocalDate): String? {
        val observed = observedFixedHolidays(date.year)
        observed[date]?.let { return it }
        ...
        if (date == nthWeekday(date.year, Month.MAY, DayOfWeek.MONDAY, 4)) return "Memorial Day"
        if (date == nthWeekday(date.year, Month.JUNE, DayOfWeek.THURSDAY, 4).minusDays(1)) return null

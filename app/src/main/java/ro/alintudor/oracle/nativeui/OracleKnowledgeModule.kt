...
class OracleKnowledgeModule(private val host: OracleNativeModule) {
    fun render(items: List<OracleKnowledgeArticle>, onOpen: (String) -> Unit, onRefresh: () -> Unit = {}) {
        ...
        hero.addView(TextView(context).apply {
            text = "Accesează articole, analize și idei\npentru investitori inteligenți."; textSize = 15f; gravity = Gravity.CENTER; setLineSpacing(host.dp(3).toFloat(), 1f); setTextColor(Color.WHITE)
        }, LinearLayout.LayoutParams(-1, host.dp(64)))
        hero.addView(Button(context).apply {
            text = "↗   DESCHIDE ALINTUDOR.RO/KNOWLEDGE"; textSize = 13f; typeface = Typeface.DEFAULT_BOLD; isAllCaps = false; setTextColor(Color.rgb(8,12,20))
        ...
        info.addView(TextView(context).apply {
            text = "Conținut independent\nNu necesită autentificare.\nSe deschide în browser."; textSize = 13f; setLineSpacing(host.dp(2).toFloat(), 1f); setTextColor(Color.rgb(205,212,225)); setPadding(host.dp(10),0,0,0)
        ...
        if (error.isNotBlank()) host.addCard("ULTIMA EROARE DE SINCRONIZARE", error)
        if (items.isEmpty()) {
            host.addCard("ARTICOLE", "Nu există încă articole în cache. Folosește REFRESH KNOWLEDGE pentru preluarea inițială.")
            return
        }
        ...
        items.forEach { article ->
            ...
            card.addView(Button(context).apply {
                text="DESCHIDE ARTICOLUL"; textSize=12f; typeface=Typeface.DEFAULT_BOLD; isAllCaps=false; setTextColor(Color.WHITE)

...
class OracleJournalModule(private val host: OracleNativeModule) {
    ...
    fun render(journal: List<OracleJournalEntry>, history: List<OracleHistoryPoint>, alerts: List<OracleAlert>) {
        ...
        if (timeline.isEmpty()) {
            host.addCard("FĂRĂ ACTIVITATE", "Nu există încă evenimente locale.")
            return
        }
    ...
    private fun addSummary(events: Int, actions: Int, activeAlerts: Int) {
        ...
        stat(row, "EVENIMENTE", events.toString(), Color.rgb(70, 185, 255))
        stat(row, "ACȚIUNI", actions.toString(), Color.rgb(255, 205, 45))
        stat(row, "ALERTE ACTIVE", activeAlerts.toString(), Color.rgb(255, 75, 60))
    ...
    private fun addDownloadButton(journal: List<OracleJournalEntry>, timeline: List<OracleTimelineItem>) {
        val button = TextView(host.root.context).apply {
            text = "DESCARCĂ JURNAL ACTIVITATE"
            textSize = 12f
    ...
    private fun exportJournal(journal: List<OracleJournalEntry>, timeline: List<OracleTimelineItem>) {
        ...
        val filename = "oracle_jurnal_activitate_$stamp.csv"
        val csv = buildString {
            append("Data/Ora,Ticker,Tip,Severitate,Titlu,Detalii\n")
            ...
            if (timeline.isEmpty()) {
                journal.sortedByDescending { it.timestamp }.take(250).forEach { e ->
                    ...
                    append(',').append(csvField("Scor ${String.format(Locale.US, "%.1f", e.score)}"))
        ...
        runCatching {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                val uri = host.root.context.contentResolver.insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI, values)
                    ?: error("Nu pot crea fișierul în Downloads/Oracle")
                try {
                    host.root.context.contentResolver.openOutputStream(uri)?.use(writer)
                        ?: error("Nu pot scrie fișierul")
            ...
            Toast.makeText(host.root.context, "Jurnal descărcat: Downloads/Oracle/$fileName", Toast.LENGTH_LONG).show()
        }.onFailure {
            Toast.makeText(host.root.context, "Export jurnal eșuat: ${it.message ?: it.javaClass.simpleName}", Toast.LENGTH_LONG).show()
        }
    }

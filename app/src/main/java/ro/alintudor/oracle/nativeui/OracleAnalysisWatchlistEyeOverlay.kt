...
object OracleAnalysisWatchlistEyeOverlay {
            ...
            for (i in 0 until content.childCount) {
                ...
                val eye = EyeView(host).apply {
                    ...
                    setOnClickListener {
                        ...
                        Toast.makeText(
                            host.root.context,
                            if (nowPresent) "$ticker adăugat în Watchlist" else "$ticker scos din Watchlist",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
    ...
    private fun collectRows(v: View, out: MutableList<ViewGroup>) {
        if (v is ViewGroup) {
            ...
            if (!desc.isNullOrBlank() && desc.contains("deschide în Analysis", ignoreCase = true)) out.add(v)
    ...
    private fun openAnalysis(activity: Activity, ticker: String) {
        ...
        runCatching {
            ...
            Toast.makeText(activity, "Nu s-a putut deschide Analysis pentru $ticker", Toast.LENGTH_SHORT).show()
        }
    }

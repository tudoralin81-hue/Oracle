...
class OraclePortfolioModule(private val host: OracleNativeModule) {
    ...
    private fun addBottomActions(p:List<OraclePosition>,j:List<OracleJournalEntry>){val row=LinearLayout(context).apply{orientation=LinearLayout.HORIZONTAL;setPadding(host.dp(2),host.dp(3),host.dp(2),0)};row.addView(btn("JURNAL ACTIVITATE",Color.rgb(55,215,255)){share("AI STOCK ORACLE — JURNAL\n"+j.sortedByDescending{it.timestamp}.take(150).joinToString("\n"){"${date.format(Date(it.timestamp))} | ${it.ticker} | ${it.action} | score=${it.score} | ${it.reason}"}})},LinearLayout.LayoutParams(0,host.dp(46),1f).apply{setMargins(0,0,host.dp(3),0)});row.addView(btn("DESCARCĂ EXCEL",Color.rgb(65,225,135)){share(csv(p,j))},LinearLayout.LayoutParams(0,host.dp(46),1f).apply{setMargins(host.dp(3),0,host.dp(3),0)});row.addView(btn("DESCARCĂ PDF",Color.rgb(255,205,65)){share(pdfText(p))},LinearLayout.LayoutParams(0,host.dp(46),1f).apply{setMargins(host.dp(3),0,0,0)});host.content.addView(row)}

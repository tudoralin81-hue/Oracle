 ...
 * It is presentation-first: Oracle values arrive as cached snapshots and are not
 * recomputed in the Android client.
 */
class OracleGrowthModule(private val host: OracleNativeModule) {
    ...
    fun render(items: List<OracleGrowthRecommendation>, fallbackNews: List<ro.alintudor.oracle.core.OracleNews> = emptyList()) {
        ...
        listOf("SHORT" to "TERMEN SCURT" to "1–10 zile bursiere", "MEDIUM" to "TERMEN MEDIU" to "2–12 săptămâni", "LONG" to "TERMEN LUNG" to "3–12 luni").forEach { (horizon, label, range) ->
    ...
    private fun addHorizon(label:String, range:String, item:OracleGrowthRecommendation?, news:List<ro.alintudor.oracle.core.OracleNews>) {
        ...
        header.addView(text(range,14f,Typeface.DEFAULT,muted,host.dp(34),3))
    ...
    private fun addRecommendation(item:OracleGrowthRecommendation, news:List<ro.alintudor.oracle.core.OracleNews>) {
        ...
        timeRow.addView(text("ANCHOR 16:00",11f,Typeface.DEFAULT_BOLD,cyan,0,0))
        ...
        company.addView(text(item.company,17f,Typeface.DEFAULT_BOLD,Color.WHITE,0,0))
        company.addView(text(item.sector,14f,Typeface.DEFAULT_BOLD,Color.rgb(160,175,205),0,5))
        ...
        metrics.addView(metric("SEMNAL", item.signal, orange),LinearLayout.LayoutParams(0,-2,1.2f))
        metrics.addView(metric("RISC", item.risk, red),LinearLayout.LayoutParams(0,-2,1f))
        metrics.addView(metric("ALOCARE MAX.", "${format(item.allocationMax)}%", orange),LinearLayout.LayoutParams(0,-2,1f))
        ...
        forecast.addView(text("Potențial estimat",18f,Typeface.DEFAULT,Color.WHITE,0,0).apply{gravity=Gravity.CENTER})
        forecast.addView(text("${if(item.forecastPct>=0)"+" else ""}${format(item.forecastPct)}%",42f,Typeface.DEFAULT_BOLD,green,0,5).apply{gravity=Gravity.CENTER})
        card.addView(forecast,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,host.dp(6),0,host.dp(10))})

        val mom=LinearLayout(host.root.context).apply{orientation=LinearLayout.HORIZONTAL;setPadding(0,host.dp(2),0,host.dp(9))}
        mom.addView(metric("MOMENTUM 5D","${format(item.momentum5D)}%",cyan),LinearLayout.LayoutParams(0,-2,1f))
        mom.addView(metric("MOMENTUM 20D","${format(item.momentum20D)}%",cyan),LinearLayout.LayoutParams(0,-2,1f))
        if(item.currentActualPct!=null)mom.addView(metric("ACTUAL","${format(item.currentActualPct!!)}%",Color.WHITE),LinearLayout.LayoutParams(0,-2,1f))
        card.addView(mom)

        card.addView(text("Ponderi (importanță)",16f,Typeface.DEFAULT_BOLD,Color.WHITE,0,4))
        addWeightGrid(card,item.weights)

        val linkedNews = news.firstOrNull { it.ticker.equals(item.ticker,true) }
        val newsTitle = if(item.newsTitle.isNotBlank()) item.newsTitle else linkedNews?.title.orEmpty()
        val source = if(item.newsSource.isNotBlank()) item.newsSource else linkedNews?.source.orEmpty()
        if(newsTitle.isNotBlank()) {
            val newsCard=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL;setPadding(host.dp(2),host.dp(11),host.dp(2),host.dp(2))}
            newsCard.addView(text("▣  ${if(source.isBlank())"NEWS" else source}",12f,Typeface.DEFAULT_BOLD,cyan,0,0))
            newsCard.addView(text(newsTitle,14f,Typeface.DEFAULT,Color.WHITE,0,5))
            card.addView(newsCard)
        }
        card.addView(text("Datele sunt informative și nu constituie recomandări de investiții.",11f,Typeface.DEFAULT,Color.rgb(125,135,155),0,11))
        host.content.addView(card,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,host.dp(13))})
    }

    private fun addWeightGrid(parent:LinearLayout, weights:List<Int>) {
        if(weights.isEmpty())return
        val columns=if(host.root.context.resources.configuration.screenWidthDp>=600)6 else 3
        val rows=(parameterNames.size+columns-1)/columns
        for(r in 0 until rows){
            ...
            for(c in 0 until columns){
                ...
                if(index>=parameterNames.size)break
                val value=weights.getOrNull(index) ?: 0
                val cell=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL;gravity=Gravity.CENTER;setPadding(host.dp(2),host.dp(8),host.dp(2),host.dp(8))}
                cell.addView(text(parameterNames[index],10f,Typeface.DEFAULT,muted,0,0).apply{gravity=Gravity.CENTER})
                cell.addView(text(if(value>0)value.toString() else "—",15f,Typeface.DEFAULT_BOLD,cyan,0,3).apply{gravity=Gravity.CENTER})
    ...
    private fun addHistory(items:List<OracleGrowthRecommendation>) {
        val label=TextView(host.root.context).apply{text="JURNAL GROWTH • ISTORIC FORECAST";textSize=17f;typeface=Typeface.DEFAULT_BOLD;letterSpacing=.06f;setTextColor(cyan);setPadding(host.dp(4),host.dp(9),host.dp(4),host.dp(8))}
        host.content.addView(label)
        items.sortedBy { it.referenceTimestamp }.forEach { item ->
            val row=LinearLayout(host.root.context).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(host.dp(14),host.dp(12),host.dp(14),host.dp(12));background=GradientDrawable().apply{setColor(bg);cornerRadius=host.dp(13).toFloat();setStroke(host.dp(1),Color.rgb(34,43,65))}}
            row.addView(text(item.ticker,16f,Typeface.DEFAULT_BOLD,Color.WHITE,0,0),LinearLayout.LayoutParams(host.dp(64),-2))
            row.addView(text("${item.horizon}\nT0 ${formatT0(item.referenceTimestamp)}",11f,Typeface.DEFAULT,muted,0,0),LinearLayout.LayoutParams(0,-2,1.4f))
            row.addView(text("Forecast\n${if(item.forecastPct>=0)"+" else ""}${format(item.forecastPct)}%",12f,Typeface.DEFAULT_BOLD,green,0,0),LinearLayout.LayoutParams(0,-2,1f))
            row.addView(text("Scor\n${item.score}/100",12f,Typeface.DEFAULT_BOLD,cyan,0,0),LinearLayout.LayoutParams(0,-2,.8f))

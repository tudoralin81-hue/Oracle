...
class OracleSimpleModule(private val host: OracleNativeModule, private val moduleTitle: String) {
    ...
    private fun renderAnalysis(){
        ...
        host.addCard("12 PARAMETRI GROWTH","Aceeași definiție și aceleași ponderi ca în Growth. News rămâne neutru în Analysis; nu sunt afișate știri.")
    ...
    private fun renderResult(r:OracleAnalysisEngine.Result?){
        ...
        top.addView(TextView(host.root.context).apply{text="${money(r.price)} • S ${r.shortScore}/100 • M ${r.mediumScore}/100 • L ${r.longScore}/100";textSize=12f;setTextColor(host.accent);setPadding(0,host.dp(5),0,0)})
        top.addView(TextView(host.root.context).apply{text="${r.signal} • RISC ${r.risk} • Alocare max ${fmt(r.allocation)}%";textSize=14f;typeface=Typeface.DEFAULT_BOLD;setTextColor(if(r.signal.contains("BUY"))Color.rgb(105,245,35) else Color.rgb(255,205,55));setPadding(0,host.dp(8),0,0)})
        host.content.addView(top,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,host.dp(10))})
        host.addSectionLabel("CEI 12 PARAMETRI")
        ...
        OracleAnalysisEngine.factorNames.forEachIndexed{i,n->
            val row=LinearLayout(host.root.context).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(host.dp(12),host.dp(9),host.dp(8),host.dp(9));background=GradientDrawable().apply{setColor(if(i%2==0)Color.rgb(7,12,23) else Color.rgb(10,16,29));cornerRadius=host.dp(8).toFloat()}}
            row.addView(TextView(host.root.context).apply{text="%02d  $n".format(i+1);textSize=13f;setTextColor(Color.rgb(210,218,232))},LinearLayout.LayoutParams(0,-2,1f))
            row.addView(TextView(host.root.context).apply{text=fmt(r.factors[i]);textSize=14f;typeface=Typeface.DEFAULT_BOLD;setTextColor(factorColor(r.factors[i]))},LinearLayout.LayoutParams(host.dp(45),-2))
            row.addView(TextView(host.root.context).apply{text="S${OracleAnalysisEngine.shortWeights[i]} M${OracleAnalysisEngine.mediumWeights[i]} L${OracleAnalysisEngine.longWeights[i]}";textSize=9f;setTextColor(Color.rgb(130,145,170));gravity=Gravity.END},LinearLayout.LayoutParams(host.dp(95),-2))
            grid.addView(row,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,host.dp(2))})
        }
        ...
        analysisLines(r).forEachIndexed{i,line->card.addView(TextView(host.root.context).apply{text="${i+1}. $line";textSize=13f;setTextColor(Color.rgb(205,213,228));setPadding(0,if(i==0)0 else host.dp(6),0,0)})}
    ...
    private fun analysisLines(r:OracleAnalysisEngine.Result):List<String>{
        ...
        val score="Oracle: SHORT ${r.shortScore}, MEDIUM ${r.mediumScore}, LONG ${r.longScore}; semnal ${r.signal}."
        ...
        return listOf(trend,mom,br,vol,sr,ichi,adx,risk,score,verdict)
    }
    ...
    private fun renderWatchlist(items:List<String>){
        ...
        items.distinct().sorted().forEach{t->val row=LinearLayout(host.root.context).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;setPadding(host.dp(15),host.dp(12),host.dp(10),host.dp(12));background=GradientDrawable().apply{setColor(Color.rgb(7,12,23));cornerRadius=host.dp(14).toFloat();setStroke(host.dp(1),Color.rgb(45,70,105))}}
    ...
    private fun addItem(title:String,body:String){val c=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL;setPadding(host.dp(16),host.dp(13),host.dp(16),host.dp(13));background=GradientDrawable().apply{setColor(Color.rgb(6,10,20));cornerRadius=host.dp(14).toFloat();setStroke(host.dp(1),Color.rgb(34,43,65))}};val row=LinearLayout(host.root.context).apply{gravity=Gravity.CENTER_VERTICAL};row.addView(TextView(host.root.context).apply{text="◆";textSize=9f;setTextColor(host.accent)},LinearLayout.LayoutParams(host.dp(22),host.dp(22)));row.addView(TextView(host.root.context).apply{text=title.uppercase();textSize=15f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE)},LinearLayout.LayoutParams(0,-2,1f));row.addView(TextView(host.root.context).apply{text="›";textSize=24f;setTextColor(host.accent)},LinearLayout.LayoutParams(host.dp(24),host.dp(30)));c.addView(row);c.addView(TextView(host.root.context).apply{text=body;textSize=14f;setTextColor(Color.rgb(175,183,201));setPadding(host.dp(22),host.dp(5),0,0)});host.content.addView(c,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,host.dp(9))})}

...
class OracleSimpleModule(private val host: OracleNativeModule, private val moduleTitle: String) {
    ...
    private fun addAnalysisItem(q: OraclePosition, trend: String) {
        ...
        val card = LinearLayout(context).apply {
            ...
            background = GradientDrawable().apply {
                setColor(Color.rgb(6,10,20)); cornerRadius = host.dp(14).toFloat(); setStroke(host.dp(1), Color.rgb(34,43,65))
            }
        }
        val row = LinearLayout(context).apply { gravity = Gravity.CENTER_VERTICAL }
        row.addView(TextView(context).apply {
            text = ticker; textSize = 18f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE)
        }, LinearLayout.LayoutParams(0, -2, 1f))
        row.addView(TextView(context).apply {
            text = "›"; textSize = 28f; setTextColor(host.accent); gravity = Gravity.CENTER
        }, LinearLayout.LayoutParams(host.dp(30), host.dp(38)))
        row.setOnClickListener { openTicker(ticker) }
        card.addView(row)
        card.addView(TextView(context).apply {
            text = "${q.status}  •  Preț ${fmtMoney(q.currentPrice)}  •  Pondere ${fmt(q.weight)}%\nP/L ${fmtMoney(q.pnl)}  •  ${fmt(q.pnlPercent)}%\n$trend"
            textSize = 14f; setTextColor(Color.rgb(175,183,201)); setPadding(0, host.dp(5), 0, host.dp(7))
        })
        val action = Button(context).apply {
            ...
            setOnClickListener {
                ...
                text = if (watchlist.contains(ticker)) "✓ ÎN WATCHLIST" else "+ ADAUGĂ ÎN WATCHLIST"
            }
        }
    ...
    private fun renderWatchlist(p: List<OraclePosition>, h: List<OracleHistoryPoint>) {
        ...
        host.addCard("WATCHLIST", if (saved.isEmpty()) "Niciun ticker selectat. Adaugă tickere din Analysis." else "${saved.size} ticker(e) salvate local • lista nu depinde de Portfolio.")
        ...
        saved.forEach { ticker ->
            ...
            val t = tr[ticker]
            addWatchlistItem(ticker, q, t?.let { "Trend ${it.direction}  •  ${fmt(it.changePct)}%" } ?: "Trend N/A")
        }
    }

    private fun addWatchlistItem(ticker: String, q: OraclePosition?, trend: String) {
        val card = LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL
            setPadding(host.dp(16),host.dp(12),host.dp(10),host.dp(12))
        ...
        info.addView(TextView(context).apply {
            ...
            textSize=14f; setTextColor(Color.rgb(175,183,201)); setPadding(0,host.dp(3),0,0)
        })
        card.addView(info, LinearLayout.LayoutParams(0,-2,1f))
        card.addView(TextView(context).apply { text="›"; textSize=30f; setTextColor(host.accent); gravity=Gravity.CENTER }, LinearLayout.LayoutParams(host.dp(34),host.dp(46)))
        card.setOnClickListener { openTicker(ticker) }
        host.content.addView(card, LinearLayout.LayoutParams(-1,-2).apply { setMargins(0,0,0,host.dp(9)) })

        val delete = Button(context).apply {
            text="ȘTERGE"; isAllCaps=false; textSize=13f; setTextColor(Color.rgb(255,90,90));
    ...
    private fun openTicker(ticker: String) {
        context.startActivity(Intent(context, OracleTickerAnalysisActivity::class.java).putExtra("ticker", ticker))
    }
    ...
    private fun renderActions(actions: List<OracleAction>) {
        ...
        val buys=actions.count{it.action.equals("BUY",true)};val sells=actions.count{it.action.equals("SELL",true)}
    ...
    private fun addItem(title:String,body:String){
        val card=LinearLayout(context).apply{orientation=LinearLayout.VERTICAL;setPadding(host.dp(16),host.dp(13),host.dp(16),host.dp(13));background=GradientDrawable().apply{setColor(Color.rgb(6,10,20));cornerRadius=host.dp(14).toFloat();setStroke(host.dp(1),Color.rgb(34,43,65))}}
        val row=LinearLayout(context).apply{gravity=Gravity.CENTER_VERTICAL}
        row.addView(TextView(context).apply{text="◆";textSize=9f;setTextColor(host.accent);gravity=Gravity.CENTER},LinearLayout.LayoutParams(host.dp(22),host.dp(22)))
        row.addView(TextView(context).apply{text=title.uppercase();textSize=15f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE)},LinearLayout.LayoutParams(0,-2,1f))
        row.addView(TextView(context).apply{text="›";textSize=24f;setTextColor(host.accent);gravity=Gravity.CENTER},LinearLayout.LayoutParams(host.dp(24),host.dp(30)))
        card.addView(row);card.addView(TextView(context).apply{text=body;textSize=14f;setTextColor(Color.rgb(175,183,201));setPadding(host.dp(22),host.dp(5),0,0)})
        host.content.addView(card,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,host.dp(9))})
    }

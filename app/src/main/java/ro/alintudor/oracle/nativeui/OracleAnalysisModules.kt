...
class OracleSimpleModule(private val host: OracleNativeModule, private val moduleTitle: String) {
    ...
    private fun renderGrowth(positions:List<OraclePosition>,history:List<OracleHistoryPoint>){
        ...
        positions.sortedByDescending{it.pnlPercent}.forEachIndexed{i,p->
            ...
            growthCard(i+1,p,t?.direction ?: "FLAT",t?.changePct ?: p.pnlPercent)
        }
        val total=positions.sumOf{it.marketValue};val pnl=positions.sumOf{it.pnl}
        val pct=if(total-pnl!=0.0)pnl/(total-pnl)*100.0 else 0.0
        ...
        val mid=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL}
        mid.addView(TextView(host.root.context).apply{text="TOTAL PORTOFOLIU (${positions.size} ACȚIUNI)";textSize=11f;setTextColor(Color.rgb(175,182,198))})
        mid.addView(TextView(host.root.context).apply{text=fmtMoney(total);textSize=20f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE);setPadding(0,host.dp(4),0,0)})
        totalCard.addView(mid,LinearLayout.LayoutParams(0,-2,1f))
        val right=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL;gravity=Gravity.CENTER_HORIZONTAL}
        right.addView(TextView(host.root.context).apply{text="TOTAL P/L";textSize=10f;setTextColor(Color.rgb(175,182,198))})
        right.addView(TextView(host.root.context).apply{text="${if(pct>=0)"+" else ""}${fmt(pct)}%";textSize=18f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.rgb(145,245,35));setPadding(0,host.dp(4),0,0)})
        totalCard.addView(right,LinearLayout.LayoutParams(host.dp(78),-2))
        host.content.addView(totalCard,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,host.dp(1),0,host.dp(10))})
    }

    private fun growthCard(rank:Int,p:OraclePosition,tickerTrend:String,change:Double){
        val card=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL;setPadding(host.dp(16),host.dp(13),host.dp(10),host.dp(11));background=GradientDrawable().apply{setColor(Color.rgb(6,10,20));cornerRadius=host.dp(15).toFloat();setStroke(host.dp(1),Color.rgb(65,105,40))}}
        val top=LinearLayout(host.root.context).apply{gravity=Gravity.CENTER_VERTICAL}
        top.addView(TextView(host.root.context).apply{text="•";textSize=25f;setTextColor(Color.rgb(145,245,35));gravity=Gravity.CENTER},LinearLayout.LayoutParams(host.dp(20),host.dp(30)))
        top.addView(TextView(host.root.context).apply{text="$rank. ${p.ticker}";textSize=21f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE)},LinearLayout.LayoutParams(0,-2,1f))
        top.addView(TrendChartView(host.root.context),LinearLayout.LayoutParams(host.dp(145),host.dp(58)))
        top.addView(TextView(host.root.context).apply{text="›";textSize=28f;setTextColor(Color.rgb(220,230,55));gravity=Gravity.CENTER},LinearLayout.LayoutParams(host.dp(20),dp(35)))
        card.addView(top)
        val divider=View(host.root.context).apply{setBackgroundColor(Color.rgb(32,40,55))};card.addView(divider,LinearLayout.LayoutParams(-1,host.dp(1)).apply{setMargins(host.dp(22),host.dp(3),host.dp(150),host.dp(5))})
        val metrics=LinearLayout(host.root.context).apply{orientation=LinearLayout.HORIZONTAL;setPadding(host.dp(22),0,0,0)}
        metrics.addView(metric("P/L", "${fmt(p.pnlPercent)}%",Color.rgb(145,245,35)),LinearLayout.LayoutParams(0,-2,1f))
        val trend=if(tickerTrend.equals("UP",true))"UP (${fmt(change)}%) ↗" else "${tickerTrend.uppercase()} (${fmt(change)}%) —"
        metrics.addView(metric("TREND",trend,if(tickerTrend.equals("UP",true))Color.rgb(145,245,35) else Color.LTGRAY),LinearLayout.LayoutParams(0,-2,1.35f))
        card.addView(metrics)
        val lower=LinearLayout(host.root.context).apply{orientation=LinearLayout.HORIZONTAL;setPadding(host.dp(22),host.dp(9),0,0)}
        lower.addView(metric("VALOARE",fmtMoney(p.marketValue),Color.WHITE),LinearLayout.LayoutParams(0,-2,1f))
        lower.addView(metric("PONDERE","${fmt(p.weight)}%",Color.WHITE),LinearLayout.LayoutParams(0,-2,1f))
        card.addView(lower)
        host.content.addView(card,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,0,0,host.dp(10))})
    }

    private fun metric(label:String,value:String,color:Int)=LinearLayout(host.root.context).apply{orientation=LinearLayout.VERTICAL;addView(TextView(host.root.context).apply{text=label;textSize=10f;setTextColor(Color.rgb(165,172,190))});addView(TextView(host.root.context).apply{text=value;textSize=16f;typeface=Typeface.DEFAULT_BOLD;setTextColor(color);setPadding(0,host.dp(3),0,0)})}

    private fun renderAnalysis(p:List<OraclePosition>,h:List<OracleHistoryPoint>){
        host.addCard("ANALYSIS","Snapshot calculat local de Oracle");if(p.isEmpty())return
        val s=OracleAnalytics.summary(p);host.addCard("PORTFOLIO","Valoare ${fmtMoney(s.value)}\nP/L ${fmtMoney(s.pnl)}  •  ${fmt(s.pnlPct)}%\nCâștigătoare ${s.winners}  •  Pierzătoare ${s.losers}\nConcentrare maximă ${fmt(s.concentration)}%  •  Risc ${s.riskLabel}")
        val tr=OracleAnalytics.trends(h).associateBy{it.ticker};p.sortedByDescending{abs(it.pnlPercent)}.take(5).forEach{q->val t=tr[q.ticker];addItem("${q.ticker}","P/L ${fmtMoney(q.pnl)}  •  ${fmt(q.pnlPercent)}%  •  pondere ${fmt(q.weight)}%\nTrend ${t?.direction ?: "N/A"}")}
    }
...
private class TrendChartView(context:android.content.Context):View(context){

...
class OracleMysticStartView(context: Context, private val onModule: (String) -> Unit) : View(context) {
    ...
    override fun onDraw(c:Canvas){
        ...
        hit.clear(); if(wide)drawCards(c,145f,385f,225f,110f,18f,time,true) else drawCards(c,18f,475f,162f,118f,10f,time,false); drawStatus(c,wide,time)
    ...
    private fun drawStatus(c:Canvas,wide:Boolean,time:Double){val q=(.5+.5*sin(time*.72)).toFloat();val top=if(wide)635f else 845f;val bottom=if(wide)718f else 930f;val left=if(wide)145f else 18f;val right=if(wide)1135f else 702f;val r=RectF(X(left),Y(top),X(right),Y(bottom));p.style=Paint.Style.FILL;p.color=Color.rgb(3,7,9);p.alpha=250;c.drawRoundRect(r,S(11f),S(11f),p);p.style=Paint.Style.STROKE;p.color=green;p.alpha=(110+115*q).toInt();p.strokeWidth=S(1.1f);c.drawRoundRect(r,S(11f),S(11f),p);val ex=if(wide)225f else 75f;val tx=if(wide)285f else 122f;val center=if(wide)640f else 360f;val rx=if(wide)745f else 410f;val shield=if(wide)1050f else 650f;miniEye(c,X(ex),Y(top+41f),S(25f),green);textLeft(c,"ORACLE READY",X(tx),Y(top+36f),S(14f),white,Typeface.DEFAULT_BOLD);textLeft(c,"Market Intelligence Active",X(tx),Y(top+59f),S(8.7f),green,Typeface.DEFAULT);p.color=gold;p.alpha=(125+120*q).toInt();p.strokeWidth=S(1.2f);c.drawCircle(X(center),Y(top+41f),S(24f+3f*q),p);for(i in -2..2){val xx=X(center+i*6f);val hh=S(8f+abs(i)*3f);c.drawLine(xx,Y(top+41f)-hh,xx,Y(top+41f)+hh,p)};textLeft(c,"LOCAL INTELLIGENCE",X(rx),Y(top+36f),S(13f),white,Typeface.DEFAULT_BOLD);textLeft(c,"Synced & Protected",X(rx),Y(top+59f),S(8.7f),green,Typeface.DEFAULT);shield(c,X(shield),Y(top+41f),S(24f),green,q)}

...
# Performance: one global OHLCV deadline, parallel enrichment.
...
# Public, process-local progress state read by the Growth loader.
...
# Replace the enrichment section with bounded parallel work.
...
# Growth loader: determinate 50-step progress, live ETA and rotating verified-attribution quotes.
...
new='''    private fun addLoadingState() {
        val card = card(18)
        ...
        host.content.addView(card, LinearLayout.LayoutParams(-1, host.dp(255)).apply { setMargins(0, 0, 0, host.dp(10)) })
        addBuildFooter()
    }
    ...
    private fun formatEta(seconds: Double): String {
        ...
        return when {
            rounded < 60 -> "$rounded sec"
            else -> "${rounded / 60} min ${rounded % 60} sec"
        }
    }'''
s=s[:start]+new+s[end:]
p.write_text(s,encoding='utf-8')
